...
Application::Application()
{
    ...
    window = SDL_CreateWindow("Window Title", WIDTH, HEIGHT, SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE);
    if (!window) {
        throw std::runtime_error("Failed to create SDL_Window! SDL error: " + std::string(SDL_GetError()));
    }
...
{
    auto vertShaderCode = readFile(VERTEX_SHADER_PATH.c_str());
    auto fragShaderCode = readFile(FRAGMENT_SHADER_PATH.c_str());
...
VkImageView Application::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) const
{
...
void Application::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    ...
    clearValues[0].color = { { srgbToLinear(0.1f), srgbToLinear(0.1f), srgbToLinear(0.1f), 1.0f } };
...
void Application::updateUniformBuffer(uint32_t currentImage)
{
    ...
    ubo.model = glm::rotate(ubo.model, time, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    ubo.projection = glm::perspective(glm::radians(75.0f), aspectRatio, 0.1f, 1024.0f);
...
std::vector<const char*> Application::getRequiredExtensions()
{
    ...
    const char* const* extensionsNames;
    extensionsNames = SDL_Vulkan_GetInstanceExtensions(&extensionCount);

    std::vector<const char*> extensions(extensionsNames,
        extensionsNames + extensionCount);
